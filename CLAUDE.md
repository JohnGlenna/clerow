...
- **Auth: OAuth 2.1, no manual keys.** The user pastes the server URL into their MCP client and the client runs Dynamic Client Registration (RFC 7591) + the authorization-code flow with **mandatory S256 PKCE**; they sign in and approve once in the browser. Clerow is its own authorization server (`app/api/oauth/{register,authorize,approve,token}`, discovery at `.well-known/oauth-{protected-resource,authorization-server}`, consent UI at `app/connect-claude`, helpers in `lib/oauth.ts`). The issued **access token is itself a Clerow API key** (`clerow_sk_…`, ~30-day `expires_at`), so the MCP resolves it with `lib/apiKeys.ts` `resolveKey` (service-role admin client) and stashes `{userId, brandId}` in `authInfo.extra`. A rotating **refresh token** (`oauth_refresh_tokens`, migration `0012`) lets the client mint a new access token silently. Schema: `api_keys`/`0007`, `oauth_clients`+`oauth_codes`/`0011`, expiry+refresh/`0012`. There is **no hand-minted-key path** — `/api/keys` only lists/revokes connections (`GET`/`DELETE`); revoke a connection in **Settings → Clerow MCP**.
- **Tools** (`clerow-web/lib/mcp/tools.ts`): the MCP is **tiered like the dashboard** — a free user is scoped to the frontier (all of **Level 1 "Foundations"** + one **Level 2** taster), a **Premium** subscriber gets the full climb (gated via `resolveCtx`→`isSubscribed`, `tierNote`/`higherLevel`). `get_visibility` (multi-model standings — the moat; `scansRemaining` is in full-scan units), `list_tasks` (the user's actionable tasks + a summary of all levels), `get_site_audit` (technical gaps; the AI-graded `l2-*` content checks are preserved across refreshes — see `refreshSiteAudit`'s merge in `lib/audit/ensure.ts`); `get_task_content` returns a deterministic **file** (robots.txt/llms.txt) or a technical fix's **diagnostic steps** via `lib/content/files.ts` `deterministicTaskContent`, else a **brief** (`buildContentBrief` in `lib/content/generate.ts`) for the calling agent to write from — and the file/brief are **grounded in the real crawled site** (`SiteCrawl` retained on `site_audit`) plus the **multi-model scan signal** (cited sources + the synthesis verdict via `loadContentSignal`/`buildScanInsight`), so paid output is richer than free; `complete_task` marks an available task done and **verifies first where it can** — `audit-*` re-crawls the site, `l2-*` re-runs the AI page-grade against the live homepage, `l4-*` fetches the task's `targetUrl` (comparison pages must name the rival) — and syncs the awarded XP to the current ladder spec; `run_full_scan` (Premium) runs the full multi-model scan (see below); `run_probe` (Premium) scans the primary prompt on Perplexity only — the cheap early signal days after shipping, before a real re-scan ~1–2 weeks later. Tasks beyond the user's tier get a friendly pointer, not an error. (Dashboard "Make content" still uses server-side `generateFixContent`/`streamFixContent`.)
- **Connect:** `claude mcp add --transport http clerow <APP_URL>/api/mcp` (no header — the client does the OAuth browser flow; in Claude Code, `/mcp` triggers it).
- **Env:** needs `SUPABASE_SERVICE_ROLE_KEY` (token/key resolution) and `ANTHROPIC_API_KEY` (content gen).
- **`run_full_scan` (Premium, budget-limited):** a subscriber can trigger the full multi-model scan (all 5 models × top prompts) directly from the MCP — same subscription + monthly budget caps as the dashboard (`isSubscribed` + `assertBudget`, reusing `scanTopPrompts` so the route and MCP can't drift). It **blocks ~1–2 min** (runs to completion within the request, then synthesizes) and returns the fresh standings + multi-model verdict. The free MCP can still only run the cheap `get_site_audit`.
