...
- **Tools** (`clerow-web/lib/mcp/tools.ts`): the MCP is **tiered like the dashboard** — a free user is scoped to the frontier (all of **Level 1 "Foundations"** + one **Level 2** taster), a **Premium** subscriber gets the full climb (gated via `resolveCtx`→`isSubscribed`, `tierNote`/`higherLevel`). `get_visibility` (multi-model standings — the moat), `list_tasks` (the user's actionable tasks + a summary of all levels), `get_site_audit` (technical gaps); `get_task_content` returns a deterministic **file** (robots.txt/llms.txt) or a technical fix's **diagnostic steps** via `lib/content/files.ts` `deterministicTaskContent`, else a **brief** (`buildContentBrief` in `lib/content/generate.ts`) for the calling agent to write from — and the file/brief are **grounded in the real crawled site** (`SiteCrawl` retained on `site_audit`) plus the **multi-model scan signal** (cited sources + the synthesis verdict via `loadContentSignal`/`buildScanInsight`), so paid output is richer than free; `complete_task` marks an available task done; `run_full_scan` (Premium) runs the full multi-model scan (see below). Tasks beyond the user's tier get a friendly pointer, not an error. (Dashboard "Make content" still uses server-side `generateFixContent`/`streamFixContent`.)
