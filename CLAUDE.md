...
- **Tools** (`clerow-web/lib/mcp/tools.ts`): the whole MCP is **free**, deliberately scoped to **Level 1 ("Foundations")** — the paid product is the dashboard (re-scan, unlock-ahead, in-dashboard content gen). `get_visibility` (multi-model standings — the moat), `list_tasks` (Level 1 tasks + a summary of all levels), `get_site_audit` (technical gaps); `get_task_content` returns deterministic robots.txt/llms.txt via `lib/content/files.ts`, else a **brief** (`buildContentBrief` in `lib/content/generate.ts` — GEO rules + brand/competitor context) for the calling agent to write from, so the free MCP spends no model call of its own; `complete_task` marks a Level-1 task done. `get_task_content`/`complete_task` reject non-Level-1 task ids with a pointer to the dashboard. (Dashboard "Make content" still uses server-side `generateFixContent`/`streamFixContent`.)
