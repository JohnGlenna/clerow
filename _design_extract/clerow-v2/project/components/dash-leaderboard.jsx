/* Leaderboard — category rivals + Clerow user race */

function PageLeaderboard() {
  const [tab, setTab] = React.useState("category"); // category | users
      ...
      <PageHead
        ...
        actions={
          <>
            ...
            <button className="btn btn--primary btn--sm" disabled>This week</button>
          </>
        }
      />
...
function BoardCategory() {
  const rows = [
    ...
    { rank: 2, name: "Linear (you)", vis: 66, pos: 2.1, sent: 92, delta: "+1.2", up: true, sw: "#F59E0B", me: true, gap: 12 },
    ...
    <>
      <div className="callout">
        ...
        <div>
          ...
          <span> AI citations aren't deterministic — they bounce ±3% day to day. We compute leaderboard from 7-day rolling averages so what you see is signal, not noise.</span>
        </div>
      </div>
          ...
          {rows.map((r) => (
            <div key={r.rank} className={`dt-row ${r.me ? "dt-row--me" : ""}`}>
              ...
              <span style={{ flex: 1.5 }} className="dt-brand">
                <span className="ex-sw" style={{ background: r.sw }}>{r.name[0]}</span>
                <span style={{ fontWeight: 700 }}>{r.name}</span>
                {r.me && <span className="ex-you">YOU</span>}
              </span>
              <span style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "center", gap: 8 }} className="dt-vis-wrap">
                <span className="dt-vis-bar"><i style={{ width: `${(r.vis / 80) * 100}%`, background: r.me ? "var(--accent)" : "var(--ink)" }} /></span>
                <span className="dt-pos" style={{ minWidth: 38, textAlign: "right" }}>{r.vis}%</span>
              </span>
              ...
              <span style={{ flex: 0.7, textAlign: "right" }} className={`dt-trend ${r.up ? "up" : "down"}`}>{r.delta}</span>
              <span style={{ flex: 1.4, textAlign: "right", fontSize: 12.5, color: "var(--ink-2)", fontWeight: 600 }}>
                {r.me ? <span style={{ color: "var(--accent-2)", fontWeight: 800 }}>+12% to pass Jira</span>
                       : r.rank === 1 ? <span style={{ color: "var(--success)" }}>Leader</span>
    ...
    <>
      <div className="callout callout--accent">
        ...
        <div>
          ...
          <span> Pass @sepy (Glide) by closing 4 quests in the next 6 days. Want your URL hidden? Toggle 'Anonymous' in settings.</span>
        </div>
          ...
          {rows.map((r) => (
            <div key={r.rank} className={`dt-row ${r.me ? "dt-row--me" : ""}`}>
              ...
              <span style={{ flex: 1.4 }} className="dt-brand">
                <span className="ex-sw" style={{ background: r.sw }}>{r.name[1] === "a" ? "?" : r.name[1].toUpperCase()}</span>
                <span style={{ fontWeight: 700 }}>{r.name}</span>
                {r.me && <span className="ex-you">YOU</span>}
              </span>
              <span style={{ flex: 1.2, fontFamily: "var(--font-mono)", fontSize: 12.5, color: "var(--ink-2)", fontWeight: 600 }}>{r.url}</span>
              <span style={{ flex: 0.7, textAlign: "right" }} className="dt-pos">⚡ {r.lvl}</span>
              <span style={{ flex: 0.7, textAlign: "right" }} className="dt-pos">🔥 {r.streak}</span>
              <span style={{ flex: 1, textAlign: "right" }} className="dt-pos">{r.xp.toLocaleString()}</span>
            </div>
          ))}
        </div>
      </div>
