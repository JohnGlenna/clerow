/* Clerow — Welcome (Duolingo-style) landing */
...
function Row({ rev, eyebrow, title, children, art }) {
  return (
...
function IllScan() {
  const rows = [["C","#10A37F",78],["A","#D97706",54],["P","#1CB0F6",40],["X","#566270",22],["G","#4285F4",14]];
  return (
    <div className="ill">
...
function AppSection() {
  return (
    <section className="app-sec">
      {MODELS.map(([l, c], i) => (
        <span key={i} className="app-float" style={{ background: c, top: `${15 + i * 14}%`, left: i % 2 ? "auto" : `${6 + i * 4}%`, right: i % 2 ? `${8 + i * 5}%` : "auto", animationDelay: `${i * .5}s` }}>{l}</span>
      ))}
      <div className="shell">
    ...
    <section className="whofor">
      <div className="shell">
        <div className="wf-head">
          <h2>Built for everyone who needs to be found.</h2>
        </div>
        <div className="wf-grid">
          {who.map(([ic, t, p], i) => (
            <div key={i} className="wf-card">
              <span className="wf-ic">{ic}</span>
              <h3>{t}</h3>
              <p>{p}</p>
            </div>
          ))}
  ...
  return (
    <div className="wl-modal-back" onClick={onClose}>
      <div className="wl-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <button className="wl-oauth" onClick={go}><GoogleG /> Continue with Google</button>
        <div className="wl-or">or</div>
        <input className="wl-input" type="email" placeholder="you@yourcompany.com" />
        <button className="btn btn-primary" style={{ marginTop: 12 }} onClick={go}>Continue with email</button>
...
function App() {
  ...
  return (
    <>
      ...
      <StatsBand />
      <Row title="Scan every AI at once."
        art={<IllScan />}>
      ...
      <Row rev title="Fix it like a game."
        art={<IllPath />}>
      ...
      <Row title="Or let your AI do it."
        art={<IllTerm />}>
