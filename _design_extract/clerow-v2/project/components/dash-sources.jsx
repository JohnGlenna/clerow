/* Sources — where AI gets its answers. Gap rows become quests. */

function PageSources({ onNavigate }) {
  const sources = [
    ...
    <>
      <PageHead
        ...
        actions={
          <>
            ...
            <button className="btn btn--primary btn--sm" onClick={() => onNavigate("quests")}>Open quests →</button>
          </>
        }
      />
      ...
      <div className="callout callout--accent">
        ...
        <div>
          ...
          <span> Prompts tell you you're losing. Sources tell you where to show up to start winning. Fix the top 3 gaps → est. <b style={{ color: "var(--accent-2)" }}>+18 visibility points</b>.</span>
        </div>
      </div>
      ...
      <div className="app-card" style={{ padding: 0, overflow: "hidden" }}>
        <div className="data-table">
          ...
          {sources.map((s, i) => {
            ...
            return (
              <div key={i} className={`dt-row ${gap ? "dt-row--invisible" : ""} ${losing ? "dt-row--losing" : ""}`}>
                <span style={{ flex: 1.8 }} className="dt-source">
                  <span className="source-fav" style={{ background: typeCol(s.type) }} />
                  <span>{s.domain}</span>
                </span>
                <span style={{ flex: 0.8 }}>
                  <span className="src-type" style={{ background: `color-mix(in oklab, ${typeCol(s.type)} 14%, white)`, color: typeCol(s.type), border: `1px solid color-mix(in oklab, ${typeCol(s.type)} 30%, transparent)` }}>
                    {s.type}
                  </span>
                </span>
                <span style={{ flex: 0.7, textAlign: "center" }} className="dt-pos">{s.cited}%</span>
                <span style={{ flex: 0.7, textAlign: "center", color: s.you === 0 ? "var(--danger)" : "var(--ink)" }} className="dt-pos">
                  {s.you === 0 ? "0" : s.you}
                </span>
                <span style={{ flex: 0.7, textAlign: "center", fontSize: 12.5 }} className="dt-rival">
                  <span style={{ display: "inline-flex", flexDirection: "column", alignItems: "center" }}>
                    <span style={{ fontFamily: "var(--font-mono)", fontWeight: 700 }}>{s.comp}</span>
                    {s.fav !== "—" && <span style={{ fontSize: 10, color: "var(--ink-3)", fontWeight: 700 }}>{s.fav}</span>}
                  </span>
                </span>
                <span style={{ flex: 1.5, fontSize: 12.5, color: "var(--ink-2)", fontWeight: 500 }}>{s.note}</span>
...
Object.assign(window, { PageSources });
